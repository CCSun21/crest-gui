...
from crest_tui.toml_builder import OptimizationParams, build_optimization_toml, toml_to_string
...
class OptimizationScreen(Screen):
    """Form for structure optimization."""

    BINDINGS = [Binding("escape", "app.pop_screen", "Back", show=True)]

    def compose(self) -> ComposeResult:
        yield Header()
        yield Label("Structure Optimization", classes="screen-title")
        with Horizontal(id="main-layout"):
            with ScrollableContainer(id="form-panel"):
                ...
                yield Label("Optimization level:")
                yield Select(options=OPTLEVEL_OPTIONS, id="optlevel", value="normal")
                yield Label("Charge:")
    ...
    def _get_params(self) -> OptimizationParams:
        return OptimizationParams(
            ...
            optlevel=str(self.query_one("#optlevel", Select).value),
    ...
    def _update_preview(self, dry_run: bool = False) -> None:
        try:
            ...
            toml_str = toml_to_string(build_optimization_toml(params))
