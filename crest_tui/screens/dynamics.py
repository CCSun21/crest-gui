...
from crest_tui.toml_builder import DynamicsParams, build_dynamics_toml, toml_to_string
...
class DynamicsScreen(Screen):
    """Form for molecular dynamics simulation."""

    BINDINGS = [Binding("escape", "app.pop_screen", "Back", show=True)]

    def compose(self) -> ComposeResult:
        yield Header()
        yield Label("Molecular Dynamics", classes="screen-title")
        with Horizontal(id="main-layout"):
            with ScrollableContainer(id="form-panel"):
                ...
                yield Label("Simulation time (ps):")
                yield Input(placeholder="50.0", value="50.0", id="time")
                yield Label("Time step (fs):")
                yield Input(placeholder="5.0", value="5.0", id="tstep")
                yield Label("Temperature (K):")
                yield Input(placeholder="298.15", value="298.15", id="temperature")
                yield Label("SHAKE algorithm (0=off, 1=H, 2=all):")
                yield Input(placeholder="2", value="2", id="shake")
                yield Label("Dump frequency (steps):")
                yield Input(placeholder="250", value="250", id="dump")
                yield Label("Threads:")
    ...
    def _get_params(self) -> DynamicsParams:
        return DynamicsParams(
            ...
            time=float(self.query_one("#time", Input).value or "50.0"),
            tstep=float(self.query_one("#tstep", Input).value or "5.0"),
            temperature=float(self.query_one("#temperature", Input).value or "298.15"),
            shake=int(self.query_one("#shake", Input).value or "2"),
            dump=int(self.query_one("#dump", Input).value or "250"),
    ...
    def _update_preview(self, dry_run: bool = False) -> None:
        try:
            ...
            toml_str = toml_to_string(build_dynamics_toml(params))
