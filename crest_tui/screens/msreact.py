...
from crest_tui.toml_builder import MSREACTParams, build_msreact_toml, toml_to_string
...
class MSREACTScreen(Screen):
    """Form for MS/React pathway calculation."""

    BINDINGS = [Binding("escape", "app.pop_screen", "Back", show=True)]

    def compose(self) -> ComposeResult:
        yield Header()
        yield Label("MS/React Reaction Pathways", classes="screen-title")
        with Horizontal(id="main-layout"):
            with ScrollableContainer(id="form-panel"):
                ...
                yield Label("Reaction temperature (K):")
                yield Input(placeholder="5000.0", value="5000.0", id="react-temp")
                yield Label("Number of fragments:")
                yield Input(placeholder="2", value="2", id="nfrag")
                yield Label("Threads:")
    ...
    def _get_params(self) -> MSREACTParams:
        return MSREACTParams(
            ...
            react_temp=float(self.query_one("#react-temp", Input).value or "5000.0"),
            nfrag=int(self.query_one("#nfrag", Input).value or "2"),
    ...
    def _update_preview(self, dry_run: bool = False) -> None:
        try:
            ...
            toml_str = toml_to_string(build_msreact_toml(params))
