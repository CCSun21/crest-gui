"""NCI (non-covalent interaction) search screen."""
...
from crest_tui.toml_builder import NCIParams, build_nci_toml, toml_to_string
...
class NCIScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        yield Header()
        yield Label("NCI Complex Search", classes="screen-title")
        with Horizontal(id="main-layout"):
            with ScrollableContainer(id="form-panel"):
                yield Label("Input structure file:")
                yield Input(placeholder="complex.xyz", id="input-file")
                yield Label("Method:")
    ...
    def _get_params(self) -> NCIParams:
        return NCIParams(
    ...
    def _update_preview(self, dry_run: bool = False) -> None:
        try:
            ...
            toml_str = toml_to_string(build_nci_toml(params))
            ...
            cmd = build_legacy_command(
                params.input_file or "complex.xyz",
                "nci",
                params.method,
                params.charge,
                params.uhf,
