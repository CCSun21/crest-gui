"""Protonation / deprotonation screen."""
...
from crest_tui.toml_builder import ProtonationParams, build_protonation_toml, toml_to_string
...
class ProtonationScreen(Screen):
    """Form for protonation / deprotonation site enumeration."""

    BINDINGS = [Binding("escape", "app.pop_screen", "Back", show=True)]

    def compose(self) -> ComposeResult:
        yield Header()
        yield Label("Protonation / Deprotonation", classes="screen-title")
        with Horizontal(id="main-layout"):
            with ScrollableContainer(id="form-panel"):
                ...
                yield Label("Mode:")
                yield Select(options=MODE_OPTIONS, id="mode", value="protonate")
                yield Label("Method:")
                ...
                yield Label("Solvent (optional):")
                yield Input(placeholder="water", id="solvent")
                yield Label("Threads:")
    ...
    def _get_params(self) -> ProtonationParams:
        return ProtonationParams(
    ...
    def _update_preview(self, dry_run: bool = False) -> None:
        try:
            ...
            toml_str = toml_to_string(build_protonation_toml(params))
